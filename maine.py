...
from aiogram.types import ReplyKeyboardMarkup, KeyboardButton, PreCheckoutQuery
...
PAYMENT_TOKEN = "5420394252:TEST:543267"
...
kb_accept = ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
kb_accept.add(KeyboardButton('/Accept'))
kb_accept.add(KeyboardButton('/Not_Accept'))
...
with open('zabbka_admin_list.txt', 'r') as f:
    text = f.read()
    print(zabka_admin_list)
zabka_admin_list = [int(x) for x in text[1:-1].split(',')]
print(zabka_admin_list)
...
Я бот который поможет тебе заработать!🤗

Ознакомся и поддтверди правила нажав на кнопку "Accept"!💸🥳'''
...
async def start_message(message: types.Message):
    ...
    await bot.send_message(chat_id=chat_id, text=START, reply_markup=kb_accept)
    await bot.send_message(chat_id=chat_id, text="Правила", reply_markup=kb_accept)


@dp.message_handler(commands=['Accept'])
async def start_message(message: types.Message):
    chat_id = message.from_user.id
    await bot.send_message(chat_id=chat_id, text="Accepted", reply_markup=kb1)
    await bot.send_message(chat_id=chat_id, text="Нажимай на кнопку регистрации ниже и начинай играть!",
                           reply_markup=kb1)


@dp.message_handler(commands=['Not_Accept'])
async def start_message(message: types.Message):
    chat_id = message.from_user.id
    await bot.send_message(chat_id=chat_id,
                           text="Правила не потверждены, ознакомпесь с правилами и нажмите на кнопку Accept",
...
async def balance_message(message: types.Message):
    ...
    if '!) ' in mes:
        if message.from_user.id in admin_list:
            ...
            index = mes.find(' ')
            u_id = mes[index + 1:]
            zabka_admin_list.append(int(u_id))
            await bot.send_message(chat_id=message.from_user.id, text=f'User: {u_id} --> Zabka Administrator')
            await bot.send_message(chat_id=u_id, text=f'You Zabka administrator!', reply_markup=kb5)
            file = open('zabbka_admin_list.txt', 'w')
            file.write(str(zabka_admin_list))
            file.close()
        elif message.from_user.id not in admin_list:
            await bot.send_message(chat_id=message.from_user.id, text="Not command!")
            await bot.send_message(chat_id=admin_list[0],
    ...
    if '!( ' in mes:
        if message.from_user.id in admin_list:
            ...
            index = mes.find(' ')
            u_id = mes[index + 1:]
            index = zabka_admin_list.index(int(u_id))
            zabka_admin_list.pop(index)
            await bot.send_message(chat_id=message.from_user.id, text=f'Zabka Administrator: {u_id} --> User')
            await bot.send_message(chat_id=u_id, text=f'You not zabka administrator!', reply_markup=kb1)
            file = open('zabbka_admin_list.txt', 'w')
            file.write(str(zabka_admin_list))
            file.close()
        elif message.from_user.id not in admin_list:
    ...
    if '! ' in mes:
        if message.from_user.id in admin_list:
            ...
            i = 1
            while i:
                await bot.send_message(chat_id=user_id[i - 1], text=ch, reply_markup=kb4)
                i += 1
            ...
            await bot.send_message(chat_id=admin_list[0],
                                   ...
                                   reply_markup=kb5)
    if '!+ ' in mes:
        if message.from_user.id in admin_list or message.from_user.id in zabka_admin_list:
            # obrez
            index = mes.find(' ')
            index2 = mes.find('*')
            ind = mes[index + 1:]
